...
A pre-generated CSV of real telemetry events from automated Concept Cascade playthroughs lives at [`reports/sample-telemetry/telemetry_events.csv`](reports/sample-telemetry/telemetry_events.csv). Useful for AI / analytics work without needing to run the game yourself.

The events were generated by **`tools/play-and-capture.js`**, a Puppeteer driver that spawns a headless Chrome, opens the game with `?bot=1`, and plays many sessions across multiple parallel pages. Six "personas" (strong_student, average_student, struggling_student, risk_taker, methodical, guesser) play with different accuracy distributions, response-time profiles, tower-build counts, and risk-taking behaviors so the dataset has natural variety. **All events come from the actual game's `telemetry.event(...)` calls — none are synthesized.**

Each row is one telemetry event with 37 columns covering all event payload fields:
...
| `tower_type`, `tile_col`, `tile_row`, `gold_spent`, `old_level`, `new_level`, `success`, `difficulty_jump` | Tower events |
...
**Event types in the dataset:** `question_answered`, `tower_placed`, `tower_upgraded`, `risky_upgrade_attempted`, `wave_started`, `wave_completed`, `enemy_killed`, `enemy_leaked`, `synergy_discovered`, `early_call`, `interest_earned`, plus per-dimension `assessment_*` summaries emitted at session end.

To regenerate the dataset (overwrites the file):

```bash
# Make sure the dev server is running first:
python3 -m http.server 8899

# Then in another shell, run the bot driver (default 50000 rows, 4 pages):
node tools/play-and-capture.js [targetRows] [parallelPages]

# Sanity check the output:
python3 tools/validate-csv.py
```

The driver depends on `puppeteer-core` and Google Chrome (the system app). Install puppeteer-core with `npm i puppeteer-core` in a temp directory and set `NODE_PATH` if needed.
